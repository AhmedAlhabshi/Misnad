...
- DETERMINISTIC CLAUSE BOUNDARY RULE (apply this consistently, the same way every time you see the same text): a separate legal or financial obligation, right, restriction, penalty, fee, or condition MUST be represented as its own separate clause entry whenever it has an independent effect — i.e. it could be removed, changed, or violated without changing whether any other provision applies. Never merge two or more independently-effective provisions into a single clause entry merely because they appear in the same paragraph, sentence, or numbered item of the source contract — paragraph boundaries in the source text are a formatting artifact, not a boundary for how many clauses to extract. Do NOT summarize an entire section into one clause when that section contains multiple separate obligations or conditions — write one clause entry per independent obligation or condition, not one entry per heading/section. For example, a paragraph that both (a) states a late-payment notice requirement and (b) caps collection costs instead of a fixed penalty describes two independent clauses, not one, even though they are adjacent; a paragraph covering both asset ownership and insurance coverage describes two independent clauses; and a paragraph stating that insurance is included for the first year while renewal in later years is a separate cost describes two independent clauses (the included benefit now, and the future cost the customer will bear later), not one merged "insurance" clause. Conversely, do not artificially fragment a single genuinely-unified provision (e.g. one condition with several sub-details that only make sense together) into multiple clauses just to inflate the count.
- Do NOT omit a clause merely because its monetary amount is already captured elsewhere in the response (e.g. in "financialObligations", "fees", "penalties", or "extractedNumbers"). A clause describes the underlying right, obligation, trigger, or condition in its own words for the user — it must still be extracted even when the number it mentions is duplicated in another array.
- Do NOT treat a bare date or an ordinary financial value alone as a clause. A date or amount only becomes a clause when it creates or is attached to a right, obligation, trigger, or condition (e.g. "notice is required after 30 days of delay" is a clause; a due date by itself with no described consequence is not).
